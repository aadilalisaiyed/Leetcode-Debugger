...
```mermaid
flowchart TD
    START([User opens index.html]) --> EDITOR[Type or edit\nPython code]
    EDITOR --> CLICK[Click ▶ Run]

    CLICK --> VALIDATE{Code\nempty?}
    VALIDATE -->|Yes| STOP1[Do nothing]
    VALIDATE -->|No| DISABLE[Disable Run button\nShow spinner]

    DISABLE --> POST[POST /run\nwith code string]

    POST --> SERVER_VALIDATE{Code\nis blank?}
    SERVER_VALIDATE -->|Yes| ERR400[Return 400\nBad Request]
    SERVER_VALIDATE -->|No| COMPILE[compile code\nto bytecode]

    COMPILE --> SETTRACE[sys.settrace\ninstall _trace]
    SETTRACE --> EXEC[exec the\ncompiled code]

    EXEC --> LINE_EVENT{line event\nfired?}
    LINE_EVENT -->|Yes| SNAPSHOT[snapshot f_locals\nfilter __ keys\nsafe_repr values]
    SNAPSHOT --> APPEND[append step\nline + variables]
    APPEND --> LINE_EVENT

    LINE_EVENT -->|No more lines| CLEARTRACE[sys.settrace None\nclear trace hook]
    EXEC -->|Exception raised| ERRORSTEP[append error step\nline + error message]
    ERRORSTEP --> CLEARTRACE

    CLEARTRACE --> RETURN[Return steps list\nas JSON]
    RETURN --> BROWSER[Browser receives\nsteps array]

    BROWSER --> HASSTEPS{steps.length\n> 0?}
    HASSTEPS -->|No| MSG[Show empty\nmessage]
    HASSTEPS -->|Yes| ENABLE[Enable Prev/Next\nRun Full buttons]

    ENABLE --> SHOW[showStep current=0]

    SHOW --> ISERROR{step has\nerror key?}
    ISERROR -->|Yes| SHOWERR[Show red error bar\nhighlight line]
    ISERROR -->|No| SHOWVARS[Render variable panel\ncompare with prev step\nhighlight changed keys]
    SHOWVARS --> HIGHLIGHT[Highlight line\nin gutter]

    HIGHLIGHT --> NAV{User clicks?}
    NAV -->|Next| INCR[current++\nshowStep]
    INCR --> SHOW
    NAV -->|Prev| DECR[current--\nshowStep]
    DECR --> SHOW
    NAV -->|Run Full| FULL[Render all steps\nas table]
    NAV -->|Clear| RESET[Reset state\nclear UI]

    style START fill:#22c55e,color:#000
    style SHOWERR fill:#ef4444,color:#fff
    style SHOWVARS fill:#3b82f6,color:#fff
    style FULL fill:#7c3aed,color:#fff
    style RESET fill:#475569,color:#fff
    style ERR400 fill:#ef4444,color:#fff
```
